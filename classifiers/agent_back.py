...
def _extract_general_frs(agentftr, rst):
    ...
    for agent, features in agentftr.get().items():
        agentsegs = filter(None,
                           re.sub('[/\-v]?[0-9][0-9.]*', '/', agent).replace(';', '/').replace('(', '/').replace(')',
                                                                                                                 '/').split(
                               '/'))
...
class AgentRuler:
    ...
    def classify(self, package):
        agentsegs = [seg.strip() for seg in filter(None, re.sub('[/\-v]?[0-9][0-9.]*', '/', package.agent).replace(';',
                                                                                                                   '/').replace(
            '(', '/').replace(')', '/').split('/'))]
