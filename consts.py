...
SQL_DELETE_HOST_RULES = 'DELETE FROM patterns WHERE paramkey IS NULL and pattens IS NULL and rule_type=%s'
SQL_SELECT_HOST_RULES = 'SELECT host, label, rule_type FROM patterns WHERE paramkey is NULL and pattens is NULL' 

SQL_INSERT_CMAR_RULES = 'INSERT INTO patterns (label, pattens, confidence, support, host, rule_type) VALUES (%s, %s, %s, %s, %s, %s)'
SQL_DELETE_CMAR_RULES ='DELETE FROM patterns WHERE paramkey IS NULL and pattens IS NOT NULL and rule_type = %s'
SQL_SELECT_CMAR_RULES = 'SELECT label, pattens, host, rule_type, confidence FROM patterns where pattens is not NULL'

SQL_DELETE_KV_RULES = 'DELETE FROM patterns WHERE paramkey IS NOT NULL and pattens IS NULL and rule_type=%s'
