...
def testHostClassifier():
    ...
    for record in records:
        ...
        if lableDist[0]:
            ...
            companyCounter += 1
    print 'appCounter:', appCounter, 'companyCounter:', companyCounter, 'total:', len(
        records), 'sum:', appCounter + companyCounter


def list_features():
  from utils import get_record_f
  from collections import defaultdict
  records = load_pkgs()
  f_counter = defaultdict(int)
  f_class = defaultdict(set)

  for record in records:
    for f in get_record_f(record):
      f_counter[f] += 1
      f_class[f].add(record.app)
      f_class[f].add(record.company)
