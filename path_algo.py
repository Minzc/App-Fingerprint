...
def _get_train_set():
	...
	train_set = []
	for app, header, hst, path, agent, name,company in sqldao.execute(query):
		package = Package()
		package.set_app(app)
		package.set_name(name)
		package.set_host(hst)
		package.set_path(path)
		package.set_add_header(header)
		package.set_agent(agent)
		package.set_company(company)
		train_set.append(package)
	return train_set
...
def host_tree(train_set = None):
	...
	sqldao = SqlDao()
	sqldao.execute('TRUNCATE TABLE rules')
