...
def classify(writedb, test_set = None):
	...
	QUERY = "select id, app, add_header, path, refer, hst, agent, company from packages where httptype=0 and hst =\'www.tripadvisor.com\' and path=\'/\' "
	
	classifiers = [header_classifier, agent_classifier]
	...
	if not test_set:
		test_set = []
		for id, app, add_header, path, refer, host, agent, company in sqldao.execute(QUERY):
			package = Package()
			package.set_id(id)
			package.set_app(app)
		...
		if id not in clsf_rst:
			...
			if clsrst:
				...
				if clsrst[0] != app.lower() and company.lower() not in companies:
					...
					# print "predict:", clsrst[0],"app:", app, "hst:", package.host,"cmp:",company.lower(),clsrst[1]
					
	print 'Finish'
	print "Number of recognized packages:", len(clsf_rst),"Wrong:", wrong,'Total:', len(test_set)
	if writedb:
